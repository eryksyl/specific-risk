...
tickers_input = st.sidebar.text_input("Enter Yahoo Tickers (comma separated)", value="UNH, MSTR, AMZN, TSLA, NVDA")
...
with c2:
    ...
    st.caption("Long-term 'Leash' detection (Engle-Granger Test)")
    
    from statsmodels.tsa.stattools import coint
    
    coint_results = []
    # Using prices reconstructed from returns for the test
    ...
    for i in range(len(tickers)):
        for j in range(i + 1, len(tickers)):
            ...
            # Engle-Granger test
            _, p_value, _ = coint(prices_df[t1], prices_df[t2])
    ...
    if coint_results:
        st.dataframe(pd.DataFrame(coint_results).set_index("Pair"), use_container_width=True)
    else:
